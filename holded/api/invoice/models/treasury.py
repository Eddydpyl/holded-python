...
class TreasuryAccountCreate(BaseModel):
    ...
    accountNumber: Optional[int] = Field(default=None, description="Account number (accounting account number)")
...
class TreasuryAccountResponse(BaseResponse):
    ...
    account: Optional[TreasuryAccount] = Field(default=None, description="Treasury account data")


class TreasuryAccountListResponse(BaseResponse):
    """Response model for a list of treasury accounts."""

    items: List[TreasuryAccount] = Field(default_factory=list, description="List of treasury accounts")
